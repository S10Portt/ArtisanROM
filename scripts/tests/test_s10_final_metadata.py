...
from s10_final_metadata import metadata
...
class FinalMetadataTests(unittest.TestCase):
    ...
    def test_missing_partition_is_rejected(self):
        (self.root/'product').rmdir()
        with self.assertRaisesRegex(ValueError, 'missing or linked work partition'):
            metadata(self.root)

    def test_stale_label_is_rejected(self):
        self.fc.write_text(self.fc.read_text()+'/vendor/removed u:object_r:vendor_file:s0\n')
        with self.assertRaisesRegex(ValueError, 'stale labels'):
            metadata(self.root)

    def test_system_as_root_module_has_no_phantom_directory(self):
        module = self.root/'module'
        (module/'system/system/etc').mkdir(parents=True)
        (module/'system/system/etc/example.conf').write_text('example')
        (self.root/'system/system').mkdir()
        fs = self.root/'configs/fs_config-system'
        fc = self.root/'configs/file_context-system'
        fs.write_text(fs.read_text()+'system 0 0 755 capabilities=0x0\n')
        fc.write_text(fc.read_text()+'/system u:object_r:system_file:s0\n')
        script = '''set -e
SRC_DIR="$1"
WORK_DIR="$2"
...
_GET_SELINUX_LABEL() { echo u:object_r:system_file:s0; }
ADD_TO_WORK_DIR "$WORK_DIR/module" system . 0 0 755 u:object_r:system_file:s0
'''
        result = subprocess.run(['bash', '-c', script, 'fixture', str(ROOT), str(self.root)],
